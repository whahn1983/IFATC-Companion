...
- **No cloud servers of ours.** The only network connections are the local link to Infinite Flight on your own Wi-Fi and requests to free, public, keyless services: NOAA aviation weather, radar/precipitation imagery (NOAA/NWS and NASA GIBS), real-world FAA Digital ATIS (via `datis.clowd.io`), and OpenStreetMap airport-surface data (via a public Overpass API). These carry only aviation identifiers or a small map area — never an account, name, or contact info.
