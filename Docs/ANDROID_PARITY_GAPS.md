...
An adversarial audit run against the iOS originals immediately afterwards found that *having* the eighteen rows and having them **right** are different claims, and four of them were reading the wrong thing. They are now fixed and the fixes are tested: "Precip source" named the last covered provider over a region with no coverage at all, where iOS says "None"; "Route conflict" printed only a bare severity and clock position, and answered "No conflict" for on-route weather being monitored — contradicting the cells the map was painting straight ahead, and hiding the discarded-reroute note that is the whole explanation for why no deviation was offered; "Rejoin fix" read the drawn map marker, so it blanked in exactly the case a pilot checks it and could print the marker's placeholder word "Rejoin", which is not a fix and appears in no flight plan; and "Sim-reported wind" was fed the *steering-filtered* wind, so a build reporting an implausible 460 kt — the one case that row exists to expose — made the row vanish rather than show the number. The route-conflict and rejoin-fix lines are now composed in `WeatherDeviationController` rather than in the UI mapper, because both need the detector, its excursion floor, and the record of a reroute discarded for solving onto the flight path.
