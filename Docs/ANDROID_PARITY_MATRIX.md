...
> **1040 tests across 88 classes, 0 failures, 0 skipped** — and the per-area counts below
> are real.
>
...
3. **100 parity gaps found by audit**, listed in `ANDROID_PARITY_GAPS.md` — 37 of them
   ...
   constructed nowhere in `:app`. **80 are now closed, including all 37 rated high**; 7
   medium and 10 low remain, each still carrying its own entry there. The rows below
...
| `:core` tests | ✅ **1040 tests, 88 classes, 0 failures, 0 skipped** |
