...
> **1020 tests across 85 classes, 0 failures, 0 skipped** — and the per-area counts below
> are real.
>
...
3. **100 parity gaps found by audit**, listed in `ANDROID_PARITY_GAPS.md` — 37 of them
   ...
   constructed nowhere in `:app`. **73 are now closed, including all 37 rated high**; 11
   medium and 13 low remain, each still carrying its own entry there. The rows below
...
| `:core` tests | ✅ **1020 tests, 85 classes, 0 failures, 0 skipped** |
