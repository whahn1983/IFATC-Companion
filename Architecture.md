...
- **LiveATCDetector / LiveATCStatus** — derives multiplayer and human-ATC-staffing context from manifest-mapped states (best-effort, signature-based). The standby guard is **per-frequency**: `LiveATCStatus.shouldStandBy(tunedTo:)` matches the staffed facility (resolved from the reported facility name via `ATCFacility.matching`) against the facility the pilot is currently tuned to, so `AppModel.companionStandby` defers to a human only while the pilot is on that controller's frequency. Tuning off a staffed frequency (or onto a sector no human is working) lifts the guard and the companion resumes covering that sector — e.g. with only Ground and Tower manned, Clearance Delivery is still available before pushback, and after departing and leaving Tower the companion picks up Departure, then Center. The guard engages **only when the staffed facility is positively identified and matches the tuned one**, so the companion reads the currently tuned frequency and gates only if that frequency is genuinely human-controlled. Ramp is never FAA ATC and ATIS is an automated broadcast, so neither is ever guarded; when the staffed facility can't be identified (only a facility count is exposed, or an unrecognised name) the companion does **not** gate — it keeps covering every sector rather than locking the pilot out of a frequency that may be uncontrolled.
