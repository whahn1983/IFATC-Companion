...
- **IFConnectClient** — the Infinite Flight Connect API v2 client, implemented over `Network.framework` TCP (always port 10112). Handles the wire protocol and raw state/command exchange. All reads go through a persistent **IFConnectFrameBuffer**: because TCP delivers a framed response (`Int32 id + Int32 length + payload`) as an arbitrary stream of chunks, every chunk is appended and a frame is only surfaced once its full declared length has arrived — so a partial response is never mistaken for a missing one, and multiple frames can share one buffer. **Every state read checks that the response frame echoes the id it asked for.** The link is one socket carrying request/response pairs, so the reply to a read that timed out arrives during the *next* read — and taking it on trust shifted every subsequent read by one, for as long as the link stayed out of step, with nothing to show for it: a `float` decodes cleanly whatever it actually measures. The result is a plausible number for the wrong state, which is worst where a reading is *interpreted* — one latitude landing in a heading slot read as proof the sim reports degrees, and pinned the aircraft symbol to north on the taxi and weather maps for the rest of the connection. Frames answering another id are now discarded (bounded by a count and the read deadline) until the read's own answer arrives, so a desync costs one read instead of every read after it, and the discards are logged to Diagnostics.
- **IFStateMappingStore** — maps the logical states the app reads (heading, track, on-ground, flight plan, …) onto concrete manifest entries by name signature, resolved at runtime so no state id is hardcoded. Candidates are filtered by **declared type** before any name is matched: a manifest carries ~1700 states plus every command, so a signature matched across all of them lands on whatever shares a word — `track` matched the *bool* `aircraft/0/is_on_flight_plan_track`, so the ground track read as 0° or 57° and went into the wind triangle as a bearing, and `parkingbrake` matched the `commands/…` entry beside the state it acts on. A measurement now only resolves onto a measurement, a flag onto a bool/int, a name onto a string, and nothing onto a command. The store also holds the connection's **radians-vs-degrees** decisions for angles — one per subsystem, because the aircraft's attitude states and `environment/wind_direction_true` are not always in the same convention, and letting them vote together pinned every heading to within 6° of north. See `docs/Weather.md`.
...
- **IFFlightPlanParser** — best-effort parser for the flight plan Infinite Flight exposes at `aircraft/0/flightplan`. It handles both serializations IF has used: the richer **JSON document** (per-fix coordinates, nested SID/STAR/approach procedure groups, and planned altitudes — from which the cruise/TOC level is recovered) and the plain **route string** (whitespace/arrow-separated). It strips departure/arrival **runway tokens** so the runway is never mistaken for the first enroute waypoint — but first records them as the plan's **departure/arrival runways** (e.g. `DPT RW22R` → departure runway `22R`), and recovers a cruise altitude from any `FLxxx`/altitude token. The cruise level is taken from the **highest planned altitude including the TOC/TOD display markers** (those sit at the cruise level), so it reports the final cruise level rather than the climbing level reached just before the top of climb. The detailed document has been observed to name a procedure while carrying none of its fixes, so it is never trusted to be complete: whenever the flat `flightplan/route` state names **more** fixes, its list wins. Those fixes are located from `flightplan/coordinates`, which is **index-aligned with the route** — both list one entry per plan item, in order — so the route is split on **commas only** (a compound marker like `DPT RW15L` is one entry containing a space, and splitting on whitespace would shift every position after it). Whatever coordinate/altitude the detailed document did recover is carried across by name. `IFConnectManager` reads it after connecting and periodically while polling, and pushes the result to `AppModel`, which merges departure/destination/waypoints/SID/STAR/approach/cruise into the active `FlightPlan` (manual overrides win for the fields the pilot can type). The **waypoint list is not one of those fields** — there is nowhere to type a route — so it always follows Infinite Flight, which is how a plan edited mid-flight (typically adding the approach after the ATIS) reaches the app. A read that momentarily loses the detailed state keeps the coordinates already known for each same-named fix, so the route line never blanks out between polls. The `DPT RW__` marker's **position** is kept as well (`departureRunwayCoordinate`) even though the marker itself is dropped from the route: it sits at the runway end, which is where the departure leg is actually flown from, so it is the origin the takeoff clearance's departure heading is measured from — falling back to the live on-ground position, then the field reference, when the plan carries no located runway. Measuring from the field instead is worth ~10° of bearing at a hub, where the threshold sits a mile or more from the field reference.
...
- **PhaseDetector** — derives the current `FlightPhase` from `AircraftState` (and flight plan context) using heuristics over altitude, speed, vertical speed, and ground state. Whether the aircraft is on the ground is **reported, never assumed**: every Connect state is its own request/response, so a snapshot can carry a position and an altitude while the on-ground read dropped. With no ground reference the detector **holds the previous phase** (only a vertical rate past ±500 fpm overrides the hold) rather than reading the missing flag as airborne — which is what used to default a taxiing aircraft to *climb*, and hand it to Center from the taxiway.
...
- **Hybrid ATC flow (`AppModel`)** — the pilot drives their own pre-departure calls via the ATC-tab buttons (clearance → pushback → engine start → taxi → ready), with read backs and check-ins always manual, while the controller's **position-based calls fire automatically**. As the departure taxi **approaches the runway**, Ground automatically hands the pilot to Tower to *monitor* — "monitor Tower on …" (`monitorTower`), the real-world red sign short of the runway — with a "monitor Tower on …" read-back (auto-tuning to Tower when enabled) and **no check-in required** (checking in anyway earns a "number one for departure"); the trigger is the surface coordinator's `approachingRunwayHandoff`, derived from the taxi route rather than any OSM feature, since OSM maps only the `aeroway=holding_position` hold-short line at the runway, not the monitor-tower line. As the aircraft then **nears the runway** while monitoring Tower — at the **same lead distance the automatic runway-crossing clearance uses** (`holdIssueMeters`, surfaced as the coordinator's `approachingRunwayLineup`) — Tower **automatically issues "line up and wait"** (`autoAdvanceMonitoringTower`) so the aircraft can make a **rolling line-up**, unless it is already on the runway (taxied straight on), in which case the line-up-and-wait is skipped. The **takeoff clearance fires automatically once the aircraft is lined up** on the assigned runway (`RunwayLineupDetector`), whether the pilot reported ready, was told to line up and wait, or is simply monitoring Tower; once airborne (`hasDeparted`) the **facility hand-offs, climb, descent, approach, cleared-to-land and taxi-in advance automatically from telemetry**. Departure works the climb to a configurable **TRACON ceiling** (default FL180) before handing to Center, which clears to cruise. The arrival mirrors this: at **top of descent** Center issues the *descend-via-STAR* (or a plain descend); descending back through the TRACON ceiling Center **hands off to Approach**; once the aircraft is **established** (autopilot APPR hold, or lined up on final and wings-level) Approach clears the published **ILS/GPS/Visual** approach — using the first altitude in the approach section of the flight plan as the intercept altitude — and **hands off to Tower** for the landing clearance. While inbound to land on Tower a **Go Around** button (`goAround()`) flies the missed approach: the pilot calls "going around", Tower turns the aircraft onto a **90° crosswind leg**, climbs it to the **pattern altitude** (3,000 ft above the field, the same elevation-aware `approachDefaultAltitude` used on the intercept), assigns **left traffic for the same runway**, and hands it back to Approach; on the pilot's Approach check-in Approach holds that altitude and clears them to **continue inbound**, and the conversation is **rewound to the approach state** so the cleared-approach → Tower → cleared-to-land sequence replays exactly as the first time (`goAroundInProgress` holds the automatic flow meanwhile). In live mode a **read-back gate** holds the automatic flow after each controller instruction so calls never fire back-to-back: the next call is withheld until the pilot reads back (any pilot transmission releases the gate), and if the pilot stays idle the controller repeats the call and asks "how do you read?" every ten seconds. The automatic flow only ever moves **forward** through the gate-to-gate order, so a flickering phase detector near the ground cannot bounce the conversation back to an earlier call. The ATC tab also exposes a **Tune Frequency** card so the pilot can change controllers manually (`tuneTo(_:)` / `contactRamp()`). In **Mock Mode** the first manual tune sets `manualTuning`, which suppresses the automatic (button-less) advance so the conversation moves only on a button press. In **live mode** manual tuning instead drives a **semi-automatic** flow (`advanceSemiAutomatic`): the controller's position-based calls and facility hand-offs still fire from telemetry, but a hand-off only prompts "contact *next* on …" and then holds (`pendingCheckInFacility`) until the pilot tunes that frequency and checks in, at which point the new controller gives its instruction. Same-frequency calls (the takeoff clearance once lined up — after a short `takeoffClearanceDelay` when stopped — the descend-via-STAR, the cleared-approach, the runway-exit) play on their own; tuning the controller you were just handed to suppresses the redundant "contact …". The **response buttons are surfaced by the tuned controller and phase** (`availableActions`): only the calls that apply now are shown — Clearance at the gate, push/start on Ramp, taxi on Ground, takeoff on Tower, the enroute/arrival requests on their controllers — and the **Tune Frequency** card likewise shows only the current and next-ahead facilities (`relevantFacilities`). Requesting **taxi while still on the departure Ramp** (`onDepartureRampPreTaxi`) only **hands the pilot to Ground** (`handOffDepartureRampToGround`) without issuing a taxi clearance; the pilot requests taxi again on Ground for the actual clearance, so the flow never auto-runs Ramp → Ground → taxi. The **Ramp** button is context-aware (`contactRamp()`): before departure it contacts Ramp for the **pushback** (using the **departure gate**), and on arrival it contacts Ramp for the **taxi-in to the gate** (`arriveAtGate()`, using the **arrival gate**). On arrival the "block-in / flight complete" call is **not** issued when the Ramp call is made — instead the companion **monitors telemetry and only declares the flight complete once the aircraft is actually parked at the gate**: stopped on the ground with the **parking brake set** (when Infinite Flight exposes it; otherwise a sustained full stop) **and tuned to the Ramp frequency** (`isParkedAtGate`). The **Ramp-frequency requirement is the map-independent gate** — a parking brake set out on an active taxiway, before the pilot has contacted Ramp for the gate, never ends the flight, whether or not the taxi map has usable data. This is enforced at a single choke point: `advanceAndPost` refuses to advance to `.parked` unless `isParkedAtGate` holds, so every telemetry- and check-in-driven path (not just the staged Ramp block-in) is covered. When the taxi map *also* resolved the gate position (`AirportSurfaceCoordinator.arrivalGateCoordinate`, the OSM stand matching the entered gate, captured when the pilot contacts Ramp) the aircraft must additionally be within ~80 m of it; otherwise the Ramp-frequency + parking-brake check stands. The **taxi map now stays visible through the Ground→Tower monitor hand-off** (retired only when the pilot reads back the line-up-and-wait / takeoff clearance, `retireDepartureTaxiMapAfterReadback`, with an airborne backstop) and **through the arrival Ramp switch** (retired at the `.parked` end-of-flight), so the surface keeps tracking to the `approachingRunwayLineup` cue and the pilot keeps the route to the runway / gate in view. Mock Mode is a scripted demo (telemetry and the synthetic surface are decoupled), so it isn't gated on the pilot tuning Ramp. The same parking-brake gate keeps `PhaseDetector` from calling a stop on the taxiway "parked". `clearFlight()` wipes the conversation/ATC state (keeping settings and the flight plan) to start a new flight. The complete call sequence is documented in [`docs/ATC-Flow.md`](docs/ATC-Flow.md).

## Session persistence (reconnect resume, saved flights)

- **SessionStateStore / SessionSnapshot** — persist the in-progress ATC session so a dropped Infinite Flight link (the pilot switched apps, the device slept, Wi-Fi blipped) resumes where the flight left off instead of re-deriving the conversation from raw telemetry. The snapshot captures the conversational state (`atcState`, the state-machine cursor, `currentFacility`, detected `phase`, `assignedAltitude`), the gate-to-gate flags (`hasDeparted`, `arrivalAnnounced`, `awaitingGateArrival`, `manualTuning`), and the transcript, backed by `UserDefaults`. `AppModel` writes it whenever the state meaningfully changes (and on a low-frequency heartbeat so `savedAt` stays fresh through long level phases), and on `startLive()` (reconnect or relaunch) it **restores** a recent, in-progress snapshot rather than resetting to a fresh flight. Completed (parked + arrival announced) and stale (older than the store's `maxAge`) snapshots are not resumed; `clearFlight()` / `resetAppData()` clear the snapshot. Two guards make the resume safe: live mode never restores a mock snapshot, and **`handle(state:)` ignores telemetry with no usable position** — Infinite Flight returns an empty snapshot during the reconnect handshake, and feeding it to `PhaseDetector` (which read a nil "on ground" as airborne and defaulted to *climb*) is exactly what previously jumped a parked aircraft straight to cruise on reconnect. A *half-read* snapshot is the same hazard one step further: reading a state is a long chain of individual socket round-trips, so a poll interrupted part-way through — as the forced reconnect on returning from the background does — returns the fields it got with the rest nil, which carries enough telemetry to pass that guard. Two changes close it: `IFConnectManager` **drops a snapshot whose poll was cancelled** rather than publishing it, and `PhaseDetector` holds the phase when no ground reference was reported (above).
- **SavedFlightStore / SavedFlight** — the pilot's library of named flights, so several flights can be in progress at once (Infinite Flight allows it) or one flight put down and picked up hours later. A slot holds the *same* `SessionSnapshot` the reconnect resume uses, so the two can never drift apart; the snapshot therefore captures the whole session rather than just the conversational cursor — the flight plan, the pilot's manual overrides (callsign, endpoints, gates), the tuned frequency and pending check-in, the read-back gate and its re-prompt count, the arrival-to-gate staging and gate coordinate, the field-elevation/liftoff references, and the ATIS reports. The **Diagnostics log** rides along too, but only in the saved-flight copy (`snapshotForSaving()`) — the auto-resume snapshot lives in `UserDefaults`, which iOS reads wholesale at launch, and five hundred log lines re-encoded into it on every state change would be a cost with no reader, since a reconnect keeps the log already in memory. Everything world-shaped is deliberately excluded and re-derived on load: telemetry (the last fix is cleared so nothing is measured from the previous flight's position), weather observations and the radar sample, the taxi map (re-established through the existing `pendingTaxiMapRestore` path on the first real fix), and device preferences. The library is a JSON file in Application Support — a flight carries its transcript and log, which is well past what belongs in a preferences plist. `loadSavedFlight` **resets the session and then applies** the snapshot rather than layering it over the live one: applying a gate flight onto a session on approach would leave `hasDeparted` true (the pre-departure ground flow skipped for good) while the forward-only flow guard refuses to walk the state machine back. Loading warns — advisory only — when the saved flight's endpoints disagree with what Infinite Flight is reporting, and when the session in progress isn't in the library. `settings.autoSaveFlights` (on by default) keeps the loaded slot current on meaningful change, deliberately *not* on the auto-resume heartbeat (a saved flight has no expiry to refresh); the binding is released by Clear/New Flight, Reset App Data, deleting the flight, and by a `startLive()` that finds nothing to resume — without that last one a fresh empty session would auto-save over the saved flight. Full detail in [`docs/SavedFlights.md`](docs/SavedFlights.md).
...
- **ChatterScriptGenerator** — deterministic, template-based generator **bounded to the
  ...
  (splitting a keyword written flush against its designator, `RY8R`, so the feeds that drop the
  space still resolve) and the generator uses a **departure** runway for Ground taxi and Tower
  takeoff/line-up and an **arrival** runway for Tower landing and Approach clearances. With no
  surface/ATIS loaded it falls back to a plausible random runway. Generic over
...
- **Mock taxi over the real surface** — at flight load Mock Mode pre-caches the **whole** origin and destination airport surfaces (`AirportSurfaceCoordinator.prepareSimulatedSurfaces`) so the simulated taxi routes over the **actual** OSM field, not a synthetic one — a realistic gate-to-runway departure and runway-exit-to-gate arrival, with the taxi map shown and the aircraft driven by the mock ticker on **both** ends. For a demo origin/destination the real field is **always** used: when a taxi begins before its extract has finished pre-caching — common for a large destination like KMSP, whose extract takes longer to fetch than a short demo takes to reach taxi-in — the surface stays **loading** and the taxi *waits* for the real field to download (`fetchRealSimulatedSurface`) rather than dropping onto the bundled synthetic surface. The taxi map / mock drive only begins once the real field is ready (`revealIfReady` gates on `status.isReady`); the map then animates in on the actual airport and Ground's generic clearance is superseded by the detailed OSM route. The synthetic `MockAirportSurface` is used only as a genuine fallback — a field with no OSM reference, or a real extract that can't be fetched (offline / no OSM data) or can't be routed — so the demo still taxis. A real surface that only arrives after the simulated drive has already started is cached for next time but not swapped in mid-drive (that would teleport the aircraft). The coordinator keeps "simulated drive" (the mock ticker vs live telemetry) and "synthetic surface" (fallback vs real OSM) as independent flags, so the demo can drive a simulated aircraft across real airport geometry.
